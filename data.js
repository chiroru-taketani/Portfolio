...
void mouseCV(int event, int x, int y, int flg, void *param)//マウスが押されたときに呼び出される関数
{
    ...
    //14号館プロジェクト
    {
        ...
        content: {
            ...
            overview: {
                catchphrase: "4画面（前、左右、下)を使用した映像",
                description: "愛知工業大学（AIT）のオープンキャンパスに向け、新入生に大学の魅力を伝えるために制作したロジェクションコンテンツ<br>企画・監督・撮影、および3D制作を担当。単なるスクリーンでの映像上映に留まらず、空間全体に映像を投影することで、来場者が「まるでキャンパス内をドローンに乗って空を飛んでいるような」没入感のある疑似体験ができる空間を作り上げた。", 
            ...
            techStack: [
                ...
                { category: "制作期間・役割", details: "1ヶ月・チーム制作5人（監督・撮影・3D制作・システム構築）" },
            ...
            moviedesign: [
                "**こだわり**",
                "単なる映像視聴ではなく「空を飛んでいる感覚」を全身で味わってもらうため、撮影手法と投影システムの工夫にこだわった",
                "**撮影の工夫**： 一定の速度と高度を保ちながらドローンで同一コースを4回（前・左右・下）撮影した",
                "**同期システムの開発**： 4方向の映像が少しでもズレると没入感が削がれてしまうため、OpenCVを用いて4台のプロジェクターを同期再生するプログラムを開発。技術面から体験のクオリティを上げた",
